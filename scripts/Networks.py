...
class PolicyGCN(torch.nn.Module):
    ...
    def forward(self, data, mask, batch=None):#mask: tensor([False, False, False, False, False, False,  True,  True]) batch: tensor([0, 0, 0, 0, 0, 0, 0, 0])
        x, edge_index, edge_weight = data.x, data.edge_index, data.edge_attr
        #x: 特征矩阵tensor形式(8x5)  edge_index:边的编号tensor([[0,1,0,2,0,3...],[1,0,2,0,3,0...]]) edge_weight: 边长的值tensor([5.3639,...])
        x = self.conv1(x, edge_index, edge_weight=edge_weight)
        x = F.relu(x)
        x = self.conv2(x, edge_index, edge_weight=edge_weight)
        x = F.relu(x)
        x = F.dropout(x)
        x = self.fully_con1(x)#全连接层 八个特征  tensor[[],[],....,[]]
        x = torch.masked_select(x.view(-1), mask)#选择最后两个特征 因为mask最后两个值是true
        batch = torch.masked_select(batch, mask)#选择最后两个值 因为mask最后两个值是true tensor([0,0])
        x = softmax(x, batch)#tensor([0.51, 0.49]) 选择两个边界节点的概率
        return x
...
class ValueGCN(torch.nn.Module):
    ...
    def forward(self, data, mask, batch=None):#mask: tensor([False, False, False, False, False, False,  True,  True]) batch: tensor([0, 0, 0, 0, 0, 0, 0, 0])
        ...
        x = F.relu(x)
        x = F.dropout(x)
        x = self.fully_con1(x)#全连接层 八个特征  8x100 tensor[[],[],....,[]]
        x = global_mean_pool(x, batch).mean(dim=1)#大概取了个平均数
        return x
