...
class SS2D(object):
    def __init__(self, config, verbose=False):
        ...
        self._sensor_params = read_sensor_params(self._config)#env.ini参数
        ...
        lo = int(self._config.getfloat('Simulator', 'lo'))
        np.random.seed(lo+1)
        x0 = float(np.random.randint(self._map_params.max_x)-self._map_params.max_x/2)#-20产生随机整数[low,high)
        np.random.seed(lo+2)
        y0 = float(np.random.randint(self._map_params.max_x)-self._map_params.max_x/2)#2
        np.random.seed(lo+3)
        theta0 = math.radians(float(np.random.randint(360)))#输入角度 返回弧度
        sigma_x0 = self._config.getfloat('Simulator', 'sigma_x0')#0.05
        sigma_y0 = self._config.getfloat('Simulator', 'sigma_y0')#0.05
        sigma_theta0 = math.radians(self._config.getfloat('Simulator', 'sigma_theta0'))#输入角度 返回弧度0.01度-->0.00017
        num_random_landmarks = self._config.getint('Simulator', 'num')#8

        seed = self._config.getint('Simulator', 'seed')#378098531
        if seed < 0:
            seed = int(time() * 1e6)
    ...
    def simulate(self, odom, core=True):#odom就是action
        # estimated_pose = self._slam.map.get_current_vehicle().pose * ss2d.Pose2(*odom)
        estimated_pose = ss2d.Pose2(*odom)#预估位姿[2,0,0]
        if not self._map_params.min_x < estimated_pose.x < self._map_params.max_x or not self._map_params.min_y < estimated_pose.y < self._map_params.max_y:
            return True#x坐标和y坐标在[-40,40]之间就返回true
        ...
        for key, m in measurements:
            if self._cleared:
                if m.range < self._environment_params.safe_distance:#安全距离-2.0
                    obstacle = True
                    self._cleared = False
                    break
            else:
